...
const database = storageVal?JSON.parse(storageVal):[]
    ...
    const id = Number(value.target.getAttribute('id'));
    console.log(id);
        const wrapperModal = document.getElementById('wrapper_modal');
        wrapperModal.classList.remove('hide');
        document.getElementById('modal_close')
            .addEventListener('click', () => {
                wrapperModal.classList.add('hide');
            });
       // document.getElementById('wrapper_modal')
        //    .addEventListener('click', () => {
         //       wrapperModal.classList.add('hide');
          //  });
        document.getElementById('save_travel_modal')
            .addEventListener('click', () => {
                const customerNew = {
                    ...
                    dateStart:document.getElementById("date_start_modal").value,
                ...
                database.splice(id, 1);
                database[id] = JSON.stringify(customerNew);
                localStorage.setItem('customers', JSON.stringify(database));
                wrapperModal.classList.add('hide');
                render();
                location.reload();
